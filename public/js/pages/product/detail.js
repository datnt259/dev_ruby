jQuery(document).ready(function() {
    ...
    $('.btnProductEdit').bind( 'click', function (e){
        $('#form_product_edit').find('label#txtName, label#txtBand, label#txtCategory, label#txtSubcategory, #material_list_edit, .btnProductEdit').addClass('hide');            
        $('#form_product_edit').find('input#name, select#slcBrand, select#slcCategory, select#slcSubcategory, #material_list, #material_list_select, .btnProductSave, .btnProductCancel').removeClass('hide');            
    }); 
    
    $('.btnProductCancel').bind( 'click', function (e){
        $('#form_product_edit').find('label#txtName, label#txtBand, label#txtCategory, label#txtSubcategory, #material_list_edit, .btnProductEdit').removeClass('hide');            
        $('#form_product_edit').find('input#name, select#slcBrand, select#slcCategory, select#slcSubcategory, #material_list, #material_list_select, .btnProductSave, .btnProductCancel').addClass('hide');            
    }); 
        ...
        submitHandler: function() {
            ...
            if (total > 100) {
                ...
                $('#material_error').hide();
					...
					$('button#btnProductSave').attr("disabled", true);
					var param = {'id': $("#txtId").val(),'name':$("#name").val(),'category_id':$("#slcCategory").val(),
							'subcategory_id':$("#slcSubcategory").val(), 'materials':materials, 'brand_id':$("#slcBrand").val()}; 
                    var url = '/products/' + $("#txtId").val();
                    $.ajax({
                        url: url,
                        type: 'PUT',
                        cache: false,
                        ...
                        success: function(data){
                            if (data) {
                                ...
                                $('label#txtName').val($("#name").val());
                                $('label#txtBand').text($("#slcBrand option:selected").text());
                                $('label#txtCategory').text($("#slcCategory option:selected").text());
                                $('label#txtSubcategory').text($("#slcSubcategory option:selected").text());
                                
                                $('div#material_list_edit').empty();
                                var select = $('#slcMaterials option:selected');
                                $.each(select, function(key,opt){
                                     $('#material_list_edit').append('<div class="row"><div class="col-md-4"><div class="option_'+ $(opt).val() +'">'+ $(opt).text() +' : </div></div><div class="col-md-3"><label id="txtMaterial">'+ materials[$(opt).val()] +' %</label></div></div>');
                                });
                                $('#material_list_edit').append('<label id="material_error" class="error"></label>');                                
                                $('#form_product_edit').find('label#txtName, label#txtBand, label#txtCategory, label#txtSubcategory, #material_list_edit, .btnProductEdit').removeClass('hide');            
                                $('#form_product_edit').find('input#name, select#slcBrand, select#slcCategory, select#slcSubcategory, #material_list, #material_list_select, .btnProductSave, .btnProductCancel').addClass('hide');            
